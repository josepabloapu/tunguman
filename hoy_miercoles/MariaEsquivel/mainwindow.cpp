...
//    qry.prepare("CREATE TABLE IF NOT EXISTS names (id INTEGER UNIQUE KEY AUTOINCREMENT=0, time INTEGER, name VARCHAR(30), team VARCHAR(30), number INTEGER, xpos INTEGER, ypos INTEGER)");
//    if(!qry.exec()) qDebug() << qry.lastError();
//    else qDebug() << "Table created!";
}
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event){
    ...
    if (event->type() == QEvent::MouseMove){
        ...
        qry.bindValue(":ypos",mouseEvent->pos().x());
